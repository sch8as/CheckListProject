    ...
    @role('admin')
        ...
        <form name="add-blog-post-form" id="add-blog-post-form" method="post" action="{{ route('users.update_roles', ['user' =>$user->id]) }}">
            ...
            <div class="mb-3 form-check form-switch">
              <input class="form-check-input" type="checkbox" id="list_reader" name="list_reader" {{($roles->contains('list_reader'))?("checked"):("")}}>
              <label class="form-check-label" for="flexSwitchCheckDefault">Can read user's lists</label>
            </div>
            <div class="mb-3 form-check form-switch">
              <input class="form-check-input" type="checkbox" id="list_limiter" name="list_limiter" {{($roles->contains('list_limiter'))?("checked"):("")}}>
              <label class="form-check-label" for="flexSwitchCheckChecked">Can limit user's lists</label>
            </div>
            <div class="mb-3 form-check form-switch">
              <input class="form-check-input" type="checkbox" id="moderator" name="moderator" {{($roles->contains('moderator'))?("checked"):("")}}>
              <label class="form-check-label" for="flexSwitchCheckChecked2">Can ban users</label>
            </div>
            <div class="mb-3 form-check form-switch">
              <input class="form-check-input" type="checkbox" id="admin" name="admin" {{($roles->contains('admin'))?("checked"):("")}}>
